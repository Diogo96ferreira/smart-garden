...
export default function SettingsPage() {
  ...
  return (
    <main className="mx-auto max-w-2xl space-y-6 p-4 text-[color:var(--color-text)] sm:p-6">
      ...
      <section className="space-y-3 rounded-xl bg-[var(--color-surface)] p-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { v: 'pt-PT', label: t('settings.languageOptions.pt') },
            { v: 'en-US', label: t('settings.languageOptions.en') },
          ].map((opt) => (
            <button
              key={opt.v}
              onClick={() => handleChangeLanguage(opt.v as Settings['locale'])}
              className={`rounded-lg px-3 py-2 text-sm shadow-sm ${
                settings.locale === opt.v
